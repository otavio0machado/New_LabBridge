...
class AnalysisState(AuthState):
    ...
    pdf_url: str = ""  # URL do PDF salvo no Cloudinary
    ...
    async def run_analysis(self):
        ...
        try:
            ...
            # Executar análise profunda (é um async generator, usar async for)
            yield
            async for _ in self.run_deep_analysis():
                yield
    ...
    async def generate_pdf_report(self):
        """Gera relatório PDF da análise e faz upload automático"""
        if not self.has_analysis:
            return
            
        try:
            filtered_missing, filtered_divergences, filtered_extras = self._get_unresolved_report_items()
            ...
            try:
                ...
                # Usar upload_pdf para melhor visualização
                url = await loop.run_in_executor(
                    None,
                    lambda: cloudinary_service.upload_pdf(tmp_path)
                )
    ...
    async def download_analysis_pdf(self):
        """Gera e baixa o PDF da análise."""
        if not self.has_analysis:
            self.error_message = "Nenhuma análise disponível para exportar."
            return
    ...
    def export_analysis_csv(self):
        """Gera CSV com todas as divergências e pendências."""
        if not self.has_analysis:
            self.error_message = "Nenhuma análise disponível para exportar."
            return
        ...
        writer.writerow(["Tipo", "Paciente", "Exame", "Valor COMPULAB", "Valor SIMUS", "Diferença"])

        def write_row(item_type: str, patient: str, exam: str, comp_value: float, simus_value: float):
            difference = (comp_value or 0) - (simus_value or 0)
            writer.writerow([
                item_type,
                patient or "",
                exam or "",
                f"{comp_value or 0:.2f}",
                f"{simus_value or 0:.2f}",
                f"{difference:.2f}",
            ])
        ...
        for item in self.value_divergences:
            write_row(
                "DIVERGENCIA_VALOR",
                getattr(item, "patient", ""),
                getattr(item, "exam_name", ""),
                float(getattr(item, "compulab_value", 0) or 0),
                float(getattr(item, "simus_value", 0) or 0),
            )

        # Exames faltantes no SIMUS
        for item in self.exams_only_compulab:
            write_row(
                "EXAME_FALTANTE_SIMUS",
                getattr(item, "patient", ""),
                getattr(item, "exam_name", ""),
                float(getattr(item, "compulab_value", 0) or getattr(item, "value", 0) or 0),
                0.0,
            )

        # Exames extras no SIMUS
        for item in self.exams_only_simus:
            write_row(
                "EXAME_EXTRA_SIMUS",
                getattr(item, "patient", ""),
                getattr(item, "exam_name", ""),
                0.0,
                float(getattr(item, "simus_value", 0) or getattr(item, "value", 0) or 0),
            )

        # Pacientes faltantes no SIMUS
        for item in self.patients_only_compulab:
            write_row(
                "PACIENTE_FALTANTE_SIMUS",
                getattr(item, "patient", ""),
                "",
                float(getattr(item, "total_value", 0) or getattr(item, "value", 0) or 0),
                0.0,
            )

        # Pacientes extras no SIMUS
        for item in self.patients_only_simus:
            write_row(
                "PACIENTE_EXTRA_SIMUS",
                getattr(item, "patient", ""),
                "",
                0.0,
                float(getattr(item, "total_value", 0) or getattr(item, "value", 0) or 0),
        ...
        filename = f"analise_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
        return rx.download(data=csv_content.encode('utf-8-sig'), filename=filename)
    ...
    async def save_current_analysis(self):
        ...
        try:
            ...
            if result.get('success'):
                ...
                # Atualizar lista de análises salvas (é um async generator)
                async for _ in self.load_saved_analyses():
                    yield
            else:
    ...
    async def load_saved_analyses(self):
        ...
        try:
            ...
            analyses = await loop.run_in_executor(
                None,
                lambda: saved_analysis_service.get_saved_analyses(tenant_id=tenant_id, limit=50)
            )
    ...
    async def load_saved_analysis(self, analysis_id: str):
        ...
        try:
            ...
            # Restaurar nomes de arquivo (tratar None como string vazia)
            self.compulab_file_name = analysis.get('compulab_file_name') or ''
            self.compulab_file_url = analysis.get('compulab_file_url') or ''
            self.simus_file_name = analysis.get('simus_file_name') or ''
            self.simus_file_url = analysis.get('simus_file_url') or ''
    ...
    async def load_analysis_by_id(self, analysis_id: str):
        """Alias para carregar análise salva por ID."""
        async for _ in self.load_saved_analysis(analysis_id):
            yield

    async def open_saved_analysis(self, analysis_id: str):
        """Carrega análise salva e redireciona para a página de análise."""
        async for _ in self.load_saved_analysis(analysis_id):
            yield
        yield rx.redirect("/analise")

    def open_analysis_pdf(self, pdf_url: str):
        """Abre o PDF de uma análise salva em nova aba."""
        if pdf_url:
            return rx.call_script(f"window.open('{pdf_url}', '_blank')")
        else:
            self.error_message = "PDF não disponível para esta análise"
            return None

    def _get_unresolved_report_items(self):
        """Filtra itens resolvidos para relatórios (CSV/PDF)."""
        ...
        return filtered_missing, filtered_divergences, filtered_extras
    ...
    async def delete_saved_analysis(self, analysis_id: str):
        ...
        try:
            ...
            if success:
                ...
                # Atualizar lista (é um async generator)
                async for _ in self.load_saved_analyses():
                    pass
            else:
